...
Reference List rows use one compact horizontal card grammar: left reorder handle when reordering is available, title/context text block, and one right-aligned compact icon action group. The handle is vertically centred with the complete card, the second line is muted context such as `Loose` or `Project: Shape ToDonut`, and desktop actions stay on the same row. On mobile, the three actions form a two-row grid at the right: Edit alone in the upper-right, then Archive and Delete beneath it. Reorder controls are represented only by the left `GripVertical` handle; do not duplicate Move Up or Move Down buttons on the right. Icon-only actions need accessible labels and tooltips, and destructive delete keeps the ruby/danger treatment.
...
Mobile bottom navigation paginates between two six-item dock pages. The primary page ends with Upcoming; the secondary page places Bakery immediately before Settings. Swiping the dock changes only the visible dock page, not the current destination. A restrained two-dot page indicator is acceptable when it uses monochrome inactive dots and the semantic information accent for the active dot.

## Floating Creation

Use `.fab` for the persistent Add action. It is a fixed circular `56px` button in the lower right, elevated above mobile bottom navigation and safe-area insets. Its compact `.fab-menu` expands upward as a vertical list of `44px` action rows. The FAB and its menu are fixed overlays and never reserve or enlarge document-flow space behind them; the mobile scroller owns one bottom safe-area reservation for the dock and controls. The menu remains monochrome, uses Lucide icons, closes on outside tap or Escape, and must not become a radial or decorative control.
...
Trash uses a compact segmented control for Deleted and Archived modes. The selected mode uses the standard selected surface plus the information accent underline. Labels must remain visible at narrow mobile widths and controls must expose accessible selected state. Its four entity tabs fit in one row at mobile widths.
...
Mobile uses a single-column shell, bottom navigation, `--space-4` to `--space-5` side padding, comfortable touch targets, wrapping task titles, and full-width quick-add submit buttons on narrow phones. Keep dock/FAB clearance on the scrolling workspace only; do not duplicate it on the outer shell.
...
Project browser-card actions remain in the same row as the title and metadata on desktop. Mobile Project cards replace the five-button group with one vertically centred hamburger at the far right; it opens a labelled, stacked action menu to the left and closes after selection, outside tap or Escape. Mobile Area cards use the same compact menu so descriptions may wrap through the text column. Reorder handles remain vertically centred against the complete card height.
...
Desktop Task row actions are one horizontal icon-only group: Edit, Promote, Process and Delete. All four actions use the same square dimensions, outline and corner radius. Delete retains destructive/ruby treatment.

At mobile shell widths, the four core Task actions form a compact two-column grid on the right of the card. Edit and Promote occupy the first row; Process and Move to Trash occupy the second. Desktop retains the horizontal action group.
...
On mobile, the page title and top-bar actions remain on one row. The hamburger stays at the top right and its panel is right-anchored. Filter-capable views place a Lucide Filter disclosure immediately to its left. View controls are initially removed from mobile layout until that disclosure is opened; when visible, the Show/Hide control is left aligned and More is right aligned on the same action row.
