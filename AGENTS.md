...
Bakery state is canonical `AppData.bakery` data (schema version 6), not a separate game store. Catalogue-data version 4 contains 18 purchased ingredients and 64 registry-driven recipes/products. Catalogue definitions live in `src/domain/bakeryCatalogue.ts`; business and information upgrades live in `src/domain/bakeryBusiness.ts`; Proofing Schedule window rules live in `src/domain/bakeryProofing.ts`; progress registries and derived intelligence live in `src/domain/bakeryProgress.ts`; pure commands live under `src/features/bakery`; and market calculations live in `src/domain/bakeryMarket.ts`. Permanent market effects are derived through `deriveEffectiveMarketModifiers`; timed Window/Bakery campaign effects are passed explicitly into pure market calculations; Product Spotlight is consumed by sale resolution in stable contract order; and Stock ingredient purchasing uses the authoritative ordinary/bulk `ingredientPackQuote`/`purchaseIngredient` path plus `supplierBasicPackQuote`/`purchaseSupplierBasicPack` for a 5-of-each shop pack at 90% of current equivalent individual price, rounded up. `INGREDIENT_IDS_IN_PURCHASE_ORDER` derives the sole ingredient sequence from supplier progression and supplier purchase order; Stock, shops and partial recipe input lists use its shared ordering helpers. Ingredients, Shops, Recipes and Products use one `bakery-section-heading` wrapper so their above and below spacing stays uniform, even when Recipes has the visual filter glyph. Kitchen presents owned then revealed-unbought recipes in one shared product-value order. Its recipe name and sell value share a line; unrevealed market value is a white Lucide Lock in that same position. Its single Bake action calls `craft` for exactly one product, and its recipe/ingredient art availability states plus product grid are presentation-only derivatives of canonical balances, supplier unlocks and production history. Stock filters held ingredients to their unlocked supplier, subdues zero quantities until hovered/focused, exposes ingredient-source tooltips and supplier tabs, and uses compact alternating shop rows. All six Bakery top tabs use their supplied navigation art at 150% of the original inline size, in tabs 140% of the compact control height; mobile shows three tabs per horizontally touch-scrollable viewport. Mobile shop identity and its cost action stay in one horizontal row, with the action right aligned. Shop tabs preserve their established width but are double-height and image-led; their textual grade is only the missing-PNG fallback. `CurrencyAmount` owns all visible Coin PNG-plus-number presentation including the enlarged shell top bar, whose Coin icon matches the hamburger's visual height. Title icons are always visible; Bakery's desktop Donut icon is 50% larger while mobile retains the shared size. Optional supplier-tab artwork belongs at `src/assets/bakery/shops/{starting,common,filling,artisan,premium}.png`; optional Basic Pack artwork belongs at `src/assets/bakery/packs/{basic,common,filling,artisan,premium}.png`; missing files deliberately use the registered art fallback. Existing contracts remain immutable. Development simulation lives under `src/dev/`; findings are in `docs/bakery-balance-report.md`.
...
Lists are first-class reference material with flat Items plus optional colour. Legacy `ReferenceListContent` is migrated into `ReferenceListEntry` records where practical, and older list records default to no colour and a stored `list-ordered` icon identifier for compatibility only. Tag scope removal and Tag deletion detach assignments atomically from Tasks, Projects, Lists, Schedule Task templates and any legacy List Item tag fields; schema 11 migration also cleans invalid stored assignments.

Related entity context uses the shared `EntityContextLine`: visible Area and Project prefixes are replaced by the established Lucide `LandPlot` and `FolderKanban` icons while accessible labels retain the entity kind, configured colour applies only to the related entity title, and multiple parents render comma-separated in Area, Project, then future-parent order. Icon-bearing entries remain in ordinary inline text flow so their title baselines match neighbouring text. Project locations derive indirect Area context for display without duplicating the stored relationship. A List's own configured colour applies to List titles in browser rows and to the `ListOrdered` icon in the active top-bar heading; that heading's List title remains primary white, and List Items never inherit the List colour.
...
Confirmed mutations show one success toast only. A feature callback that presents an action-bearing confirmation, such as an Undo notice, must suppress the generic `commit` success toast for that operation.
