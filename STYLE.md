...
When an entity is shown as another record's parent context, replace visible `Area:` and `Project:` prefixes with the established Lucide `LandPlot` and `FolderKanban` icons, retain an accessible entity-kind label, and colour only the entity title with that entity's configured colour. Multiple parent contexts share one comma-separated line in hierarchy order: Area, then Project, then any future parent types. A Project-linked record includes the Project's Area as indirect context. Lists with a configured colour render their own title in that colour in List rows; in an active List top-bar heading, apply the colour to the leading `ListOrdered` Lucide icon while keeping the List title primary white. The Back, leading List and trailing Quantifier icons use one shared size at 75% of the page-title font and keep the same standard gap from the title text on both sides. List Item text does not inherit the List colour.

The blank first swatch in List Appearance is the selected no-colour state and clears an existing List colour on Save.

Reference List detail headers use Back, title/location/Tags and compact icon actions. The top-bar application hamburger menu includes `Copy` when a List detail is open; it copies active Item titles only in manual order as carriage-return-separated clipboard text. Archived presentation removes edit controls rather than disabling them. A drag destination's blue insertion line clears on both drop and drag end.

Reference List Item reorder controls use the existing Lucide chevrons immediately before Edit and Delete. The first Item retains a disabled Move Up slot at zero opacity, and the last Item retains a disabled Move Down slot at zero opacity, so Edit and Delete keep the same columns on every Item.
...
Trash uses a compact segmented control for Deleted and Archived modes. It lives in the application top bar immediately left of the application menu because its mode changes the Trash screen's meaning. The selected mode uses the standard selected surface plus the information accent underline. Labels must remain visible at narrow mobile widths and controls must expose accessible selected state.
...
On mobile, the page-title row uses the same `--space-4` to `--space-5` inset at the top as at its sides. The title, its associated Lucide destination icon and all top-right controls are vertically centred in one touch-sized row; the icon gains `0.75rem` leading and `0.5rem` trailing breathing room, and title text reserves the controls' horizontal footprint. The first content block begins after one ordinary component gap, including when it has a section heading. The fixed dock uses the opaque primary surface. Shared mobile list clearance is the dock height plus one card gap and half a compact card, so the final card can sit near the usable lower edge while its pop-out actions remain available. The Donuts gallery relies on this shared clearance and adds no extra trailing footer space.
...
Mobile card leading padding is intentionally half the ordinary card inset where a reorder handle leads the row. Handles remain vertically centred. Task, List, Project, Area, Priority and Status actions use the shared vertically centred hamburger menu, opening left and closing after selection, outside tap or Escape.
...
Bakery keeps the dark monochrome application shell and uses registered ingredient/product art as restrained focal colour. Its tabs reuse the compact selected-surface and blue-underlined tab treatment used by Trash. All six top tabs pair their supplied navigation PNG with the label; the leading art is 50% larger than the original inline treatment and the tab is 70% taller to accommodate it. At mobile widths, the tab strip becomes a touch-scrollable rail showing three tabs per viewport. Stock Ingredients, Kitchen Products and Display For Sale use compact, start-aligned wrapping rails with a six-item maximum width; they wrap naturally between three and six items per desktop row. Stock ingredients pair their icon and bold quantity with a regular-weight display name. Zero-stock ingredients are at 50% opacity and return to full opacity over 200ms on hover or focus. Hover/focus tooltips and mobile taps expose each ingredient's name and source. Ingredient presentation always follows supplier progression and its within-supplier purchase order, including partial recipe ingredient lists. Its Shops area is a second compact tab strip, with locked supplier tabs visually subdued until their normal purchase prerequisite is met; supplied tab art is grayscale while locked. Shop tabs use the same 70%-taller height as the top tabs, icon-and-label controls using the same icon scale as Bakery's top tabs; their labels are Basic, Common, Filling, Artisan and Premium. Locked shops expose their Coin and supplier prerequisites through the shared requirements-tooltip treatment. Shop rows are compact icon, muted pack quantity and ingredient-name rows with fixed-width centred cost actions; on mobile the identity content and cost action remain aligned in one horizontal row. Every shop's final Basic Pack uses the same row treatment, shows `5x each of the above`, and may use its own PNG while retaining the shared fallback. Even rows use a subtle black translucent band to support scanning. Resource, shop, recipe and contract sections use dividers and compact rows rather than nested card grids. Missing art uses one consistent outlined Lucide fallback at the registered asset key, allowing PNG replacement without component changes.

Coin amounts always render with the registered Coin PNG followed by a tabular number. The current balance is persistent in the application top bar immediately left of the hamburger menu, including on mobile. Its Coin art aligns to the hamburger's visible icon height and its enlarged value remains a high-priority element.

Page-title Lucide icons are present on desktop and mobile. Desktop uses the standard navigation-icon size except Bakery, whose Donut icon is 50% larger; mobile keeps the shared standard title-icon size.
# Bakery catalogue disclosure

Long Bakery catalogues use native collapsible supplier and recipe-category sections. Recipe categories stay as ordinary rows while only Starting recipes are owned; once a recipe in another category is owned, each populated category starts collapsed on arrival. Its summary has a 20%-opaque black background, places the category title and muted `Bake: N, Recipes: N` counts on the left, and keeps its down/up chevron a `1rem` inset from the right. Category dividers appear between sections, while the final category relies on the Recipes section divider to avoid a duplicate rule. Summary rows retain the standard touch target, show a concise state/count, and cards keep art at the established 40px size. Ingredient and recipe secondary details use muted text; long names wrap rather than forcing horizontal overflow.

# Bakery Kitchen

Kitchen has two vertically divided sections: Recipes first, then Products. Recipes render all owned recipes before revealed unbought recipes, while both groups use the shared global product-value order. Each recipe is a compact two-row record: product art, then name and market value on the same line with one Bake or Buy Recipe action above; a supplier-order ingredient equation below. Unowned recipes replace that price position with a primary-white Lucide Lock rather than text. A Bake action always produces one item and is disabled when ingredients are insufficient. Product art is black when never produced or unbought, grey when previously produced but currently uncraftable, and full colour when craftable. Ingredient equation art stays coloured when held, becomes grey at zero available stock, and black when its supplier is locked. Products uses the shared Stock-style wrapping rail, omits zero quantities, shows the regular-weight product name after the bold quantity, and exposes a name tooltip.

Ingredients, Shops, Recipes and Products use one shared Bakery section-heading wrapper with identical upper and heading-to-content spacing. An optional heading-end visual such as Kitchen's filter glyph must not change that rhythm.
...
Grouping headings use ordinary section-heading typography. Empty groups are omitted. Closed sections use section-level structure and a non-colour cue; opacity alone is not sufficient. Deferred sections begin collapsed behind a full-width, tappable heading with a leading Lucide ChevronDown; expanding rotates the chevron and shows ordinary unhighlighted Task rows.
...
Desktop Task row actions are one horizontal icon-only group: Edit, Process, Delete. All three actions use the same square dimensions, outline and corner radius. Delete retains destructive/ruby treatment. Mobile uses the shared card hamburger menu instead.
