...
begin
  ...
  delete from public.app_snapshot_operations as stale
  where stale.operation_id in (
    select removable.operation_id
    from public.app_snapshot_operations as removable
    where removable.operation_id not in (
      select retained.operation_id
      from public.app_snapshot_operations as retained
      order by retained.created_at desc, retained.operation_id desc
      limit 16
    )
  );
