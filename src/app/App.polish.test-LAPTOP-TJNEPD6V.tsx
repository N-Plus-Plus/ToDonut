...
import { AreaRow, CardActionMenu, DEFAULT_TAG_SCOPES, ListRow, ProjectRow, TASK_COMPLETION_EXIT_MS, TaskViewPanel, entityDetailBackAction, entityDetailBackTarget, rebaseAppData, settingsSubsectionShowsBack, withInAppBack } from "./App";
...
describe("mobile card action menu", () => {
  afterEach(() => cleanup());

  it("opens to labelled actions, runs the chosen action, and closes", () => {
    const edit = vi.fn();
    render(
      <CardActionMenu
        label="Open actions for Example"
        actions={[{ id: "edit", label: "Edit", icon: <span aria-hidden="true">E</span>, onSelect: edit }]}
      />,
    );

    fireEvent.click(screen.getByRole("button", { name: "Open actions for Example" }));
    expect(screen.getByRole("menu", { name: "Open actions for Example" })).toBeInTheDocument();
  ...
  it("closes when the user taps outside the card menu", () => {
    render(
      <CardActionMenu
        label="Open actions for Example"
        actions={[{ id: "edit", label: "Edit", icon: <span aria-hidden="true">E</span>, onSelect: vi.fn() }]}
      />,
    );
  ...
  it("connects Project and Area lifecycle actions to their compact menus", () => {
    const data = createSeedData();
    ...
    fireEvent.click(screen.getByRole("button", { name: `Open actions for ${project.title}` }));
    expect(screen.getByRole("menuitem", { name: "Edit" })).toBeInTheDocument();
    expect(screen.getByRole("menuitem", { name: "Demote to Task" })).toBeInTheDocument();
    ...
    fireEvent.click(screen.getByRole("button", { name: `Open actions for ${area.title}` }));
    expect(screen.getAllByRole("menuitem").map((item) => item.textContent)).toEqual(["Edit", "Archive", "Move to Trash"]);
  });
});
