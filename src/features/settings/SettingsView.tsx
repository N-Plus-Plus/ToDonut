...
export function ConfigRow({ name, color, marker, metadata, isDefault, index, count, move, moveRelative, edit, remove, className = "", reorderId = String(index), reorderScope = "configuration", showColor = true, nameColor, inlineMetadata = false }: { name: string; color: string; marker?: ReactNode; metadata: string[]; isDefault?: boolean; index: number; count: number; move?: (direction: -1 | 1) => void; moveRelative?: (draggedId: string, targetId: string, placement: "before" | "after") => void; edit: () => void; remove?: () => void; className?: string; reorderId?: string; reorderScope?: string; showColor?: boolean; nameColor?: string; inlineMetadata?: boolean }) {
  ...
  return <div className={`status-row ${className} ${showColor ? "" : "status-row--without-swatch"} ${dragging ? "dragging" : ""} ${dragOver ? `drop-${dragOver}` : ""}`} onDragOver={(event) => { if (!move) return; event.preventDefault(); const bounds = event.currentTarget.getBoundingClientRect(); setDragOver(event.clientY >= bounds.top + bounds.height / 2 ? "after" : "before"); }} onDragLeave={() => setDragOver(null)} onDrop={(event) => { const placement = dragOver ?? "before"; setDragOver(null); if (!move || event.dataTransfer.getData("text/config-scope") !== reorderScope) return; const draggedId = event.dataTransfer.getData("text/config-id"); if (moveRelative && draggedId && draggedId !== reorderId) { moveRelative(draggedId, reorderId, placement); return; } const from = Number(event.dataTransfer.getData("text/config-index")); if (!Number.isFinite(from) || from === index) return; move(from < index ? 1 : -1); }}>{move && <button type="button" className="drag-handle" draggable aria-label={`Reorder ${name}`} title="Drag or use the arrow keys to reorder" onDragStart={(event) => { setDragging(true); event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/config-index", String(index)); event.dataTransfer.setData("text/config-id", reorderId); event.dataTransfer.setData("text/config-scope", reorderScope); }} onDragEnd={() => setDragging(false)} onKeyDown={reorderWithKeyboard}><GripVertical aria-hidden="true" /></button>}{showColor && (marker ? <span className="status-icon-marker" aria-hidden="true">{marker}</span> : <span className="status-swatch status-swatch--plain" style={{ "--status-colour": color } as CSSProperties} aria-hidden="true" />)}<div className={`status-row__main ${inlineMetadata ? "status-row__main--inline" : ""}`}><strong style={nameColor ? { color: nameColor } : undefined}>{name}</strong><div className="task-meta">{metadata.map((item) => <span key={item} className="badge">{item}</span>)}{isDefault && <span className="badge success">Default</span>}</div></div><div className="status-row__actions">{move && <><button type="button" className="icon-button button ghost" aria-label={`Move ${name} up`} disabled={index === 0} onClick={() => move(-1)}><ArrowUp aria-hidden="true" /></button><button type="button" className="icon-button button ghost" aria-label={`Move ${name} down`} disabled={index === count - 1} onClick={() => move(1)}><ArrowDown aria-hidden="true" /></button></>}<button type="button" className="icon-button button ghost" aria-label={`Edit ${name}`} onClick={edit}><Pencil aria-hidden="true" /></button>{remove && <button type="button" className="icon-button button danger" aria-label={`Delete ${name}`} onClick={remove}><Trash2 aria-hidden="true" /></button>}</div></div>;
}
