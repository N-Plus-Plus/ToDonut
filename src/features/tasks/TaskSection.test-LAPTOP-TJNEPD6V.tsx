...
describe("TaskRow", () => {
  ...
  it("uses icon actions and keeps interactive clicks out of row selection", () => {
    ...
    const coreActions = actions.querySelector(".task-row-actions__core")!;
    expect(Array.from(coreActions.children).map((button) => button.getAttribute("aria-label"))).toEqual([
      "Edit Task",
      `Promote ${task.title} to Project`,
      "Process Task",
      "Move Task to Trash",
    ]);
