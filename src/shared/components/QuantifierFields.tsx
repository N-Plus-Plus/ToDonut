  ...
  return <div className="quantifier-fields">
    {definitions.map((definition) => {
      ...
      return <label className="form-row" key={definition.id}>
        ...
        <select className="field" value={value[definition.id] ?? ""} onChange={(event) => {
          const next = { ...value };
          ...
          {[...definition.options].sort((a, b) => a.order - b.order || a.name.localeCompare(b.name)).map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}
        </select>
      </label>;
    })}
  </div>;
