...
Bakery is a dedicated destination with a labelled desktop navigation entry and a secondary-page mobile dock entry immediately before Settings. It participates in browser history and Back navigation.
...
The authenticated shell header uses a compact hamburger application menu. The trigger has accessible label `Open application menu`, closes on outside click, Escape, route change and after actions, and restores focus to the trigger. Desktop menu items are Export and Sign out. Bakery remains available through desktop left navigation and the secondary mobile dock and must not duplicate as a header action. Mobile menu includes Sign out; Export remains desktop-only and absent from mobile focus order. On mobile, the hamburger stays at the top right on the same row as the page label, and its panel opens inward from the right edge.
...
At mobile shell widths, view controls are collapsed initially and are revealed or hidden by an app-top-bar Lucide Filter button immediately left of the hamburger. The disclosure resets closed on route changes. When revealed, the visibility toggle stays at the left of its action row and More stays at the right of that same row.
...
On mobile only, the four core Task actions use a two-column grid at the right of the card: Edit and Promote on the first row, Process and Move to Trash on the second. Desktop retains the horizontal action row.
...
# 2026-07-23 mobile card-layout addendum

Mobile list surfaces reserve bottom clearance exactly once on the scrolling workspace. The fixed dock, Add button and expanded Add menu do not contribute to document-flow height, so the last card has only the intentional clearance needed to remain reachable above those controls.

Project cards show title and details in the main column and one vertically centred hamburger at the far right. The hamburger opens the lifecycle-valid Project actions as a labelled vertical menu to its left; selecting an action, tapping elsewhere or pressing Escape closes it. Area cards use the same compact interaction so their descriptions may span multiple lines. Desktop retains inline Project and Area action buttons.

Reference List cards retain three direct mobile actions in a compact right-side two-row grid: Edit occupies the upper-right cell, with Archive then Move to Trash beneath it. Every reorder handle is vertically centred against the complete expanded card.

Trash displays all four entity tabs in one row at mobile width. All Task cards, including those inside Project and Area detail, continue through the shared Task row presentation and mobile two-by-two action grid. Task-bearing screens also share the initially collapsed Filter disclosure and the application title row with Filter immediately before the right-aligned hamburger.
